...
class TreeView(object):
    ...
    def __init__(self, master, tree=None,
    ...
    def _createGUI(self):
        ...
        self.continue_button = Button(
            self.master, text="Continue", underline=0,
            command=self._continue_callback)
        self.continue_button.pack(side='left')

        # TODO enable/disable buttons
        self.previous_button = Button(
            self.master, text="Prev", underline=0,
            command=self._previous_callback)
        self.previous_button.pack(side='left')

        self.next_button = Button(
            self.master, text="Next", underline=0,
            command=self._next_callback)
        self.next_button.pack(side='left')

        # global shortcuts
        self.master.bind('<Escape>', self._close_callback)
        self.master.bind('<c>', lambda e: self.continue_button.invoke())
        self.master.bind('<p>', lambda e: self.previous_button.invoke())
        self.master.bind('<n>', lambda e: self.next_button.invoke())
    ...
    def _close_callback(self, event=None):
        self.master.destroy()      # TODO or use destroy() (quit kills tcl)
        self.exit = True
    ...
    def _view(self, new_snapshot_index=None):
        ...
        def draw(f=1):
            ...
            arrow_length = 1.5*self.node_radius   # TODO setting
...
if __name__ == '__main__':
    ...
    print(universe)
    universe = [10, 18, 16, 14, 0, 17, 11, 2, 3, 9, 5, 7, 4, 19, 6, 15, 8, 1, 13, 12]
